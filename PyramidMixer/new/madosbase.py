...
class ChunkPatchMixer(nn.Module):
    def __init__(self, dim, chunk_size=60, heads=4, drop_path=0.0, dropout=0.0):
        ...
        self.chunk_h, self.chunk_w = best_factor_pair(chunk_size)  # для 60 -> 6 x 10
    ...
    def _pad_spatial(self, x):
        b, c, h, w = x.shape
        pad_h = (self.chunk_h - h % self.chunk_h) % self.chunk_h
        pad_w = (self.chunk_w - w % self.chunk_w) % self.chunk_w
        if pad_h or pad_w:
    ...
    def _window_partition(self, x):
        b, c, h, w = x.shape
        wh, ww = self.chunk_h, self.chunk_w
        nh, nw = h // wh, w // ww

        x = x.reshape(b, c, nh, wh, nw, ww)
        x = x.permute(0, 2, 4, 3, 5, 1).contiguous()
        windows = x.reshape(b * nh * nw, wh * ww, c)
        return windows, (b, c, h, w, nh, nw)

    def _window_reverse(self, windows, meta):
        b, c, h, w, nh, nw = meta
        wh, ww = self.chunk_h, self.chunk_w

        x = windows.reshape(b, nh, nw, wh, ww, c)
    ...
    def forward(self, x):
        ...
        x, orig_hw, (pad_h, pad_w) = self._pad_spatial(x)
        windows, meta = self._window_partition(x)

        bn, n, c = windows.shape  # bn = batch * num_windows

        y = self.norm1(windows)
        y, _ = self.attn(y, y, y, need_weights=False)
        windows = windows + self.drop_path(y)

        y = self.norm2(windows).reshape(bn, self.chunk_h, self.chunk_w, c)
        y = y.permute(0, 1, 3, 2).contiguous().reshape(bn * self.chunk_h, c, self.chunk_w)
        y = self.mlp1(y)
        y = y.reshape(bn, self.chunk_h, c, self.chunk_w).permute(0, 1, 3, 2).contiguous()
        y = y.reshape(bn, self.chunk_size, c)
        windows = windows + self.drop_path(y)

        y = self.norm3(windows).reshape(bn, self.chunk_h, self.chunk_w, c)
        y = y.permute(0, 2, 3, 1).contiguous().reshape(bn * self.chunk_w, c, self.chunk_h)
        y = self.mlp2(y)
        y = y.reshape(bn, self.chunk_w, c, self.chunk_h).permute(0, 3, 1, 2).contiguous()
        y = y.reshape(bn, self.chunk_size, c)
        windows = windows + self.drop_path(y)

        y = self.channel_mlp(self.norm4(windows))
        windows = windows + self.drop_path(y)

        x = self._window_reverse(windows, meta)
