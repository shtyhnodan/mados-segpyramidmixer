...
def smooth_one_hot(labels, num_classes=100, smoothing=0.0):
    with torch.no_grad():
...
def train(model, run_name, seed=42):
    ...
    if os.path.exists(checkpoint_path):
        ...
        top5_accs = checkpoint.get("top5_accs", [])
    ...
    def evaluate():
        ...
        for x, y in test_loader:
            ...
            correct5 += top5.eq(y.unsqueeze(1)).any(dim=1).sum().item()
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        ema.apply(model)
        val_loss, acc, top5_acc = evaluate()
        ema.restore(model)
        ...
        print(
            ...
            f"acc={acc:.4f}, top5={top5_acc:.4f}, lr={current_lr:.6e}"
        )
    ...
    return val_accs, top5_accs, train_losses, val_losses, lrs
...
if __name__ == "__main__":
    ...
    set_seed(SEED) 
    mixer = MLPMixer()
    acc_mixer, top5_mixer, loss_mixer, valloss_mixer, lr_mixer = train(mixer, "mixer_c100", seed=SEED)

    print("\nTraining Mixer + Attention...")
    set_seed(SEED)
    hybrid = HybridModel()
    acc_hybrid, top5_hybrid, loss_hybrid, valloss_hybrid, lr_hybrid = train(hybrid, "hybrid_c100", seed=SEED)
    ...
    plt.figure()
    plt.plot(top5_mixer, label="Mixer")
    plt.plot(top5_hybrid, label="Mixer+Attention")
    plt.legend()
